...
import ca.gov.dtsstn.passport.api.data.DeliveryMethodCodeRepository;
...
/**
 * Service class to handle {@link DeliveryMethodCode} interactions.
 */
@Service
@CacheConfig(cacheNames = { "delivery-method-codes" })
public class DeliveryMethodCodeService {

	private final DeliveryMethodCodeMapper mapper;

	private final DeliveryMethodCodeRepository repository;

	public DeliveryMethodCodeService(@Lazy DeliveryMethodCodeMapper mapper, DeliveryMethodCodeRepository repository) {
	...
	@Cacheable(key = "{ 'id', #id }")
	public Optional<DeliveryMethodCode> read(String id) {
		Assert.hasText(id, "id is required; it must not be null or blank");
		return repository.findById(id).map(mapper::fromEntity);
	}

	@Cacheable(key = "{ 'cdoCode', #cdoCode }")
	public Optional<DeliveryMethodCode> readByCdoCode(String cdoCode) {
	...
	@Cacheable(key = "{ 'all' }")
	public List<DeliveryMethodCode> readAll() {
		return repository.findAll().stream().map(mapper::fromEntity).toList();
	}

	@Cacheable(key = "{ 'isActive', #isActive }")
	public List<DeliveryMethodCode> readAllByIsActive(boolean isActive) {
